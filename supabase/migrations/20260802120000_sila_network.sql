-- ════════════════════════════════════════════════════════════════════════════
-- Tabibo · Sila — le réseau des confrères
--   « Sila » (صِلة) : le lien, la relation. Le réseau professionnel qui relie
--   les cabinets Tabibo entre eux.
--
...
create or replace view public.sila_directory as
  select
...
grant select on public.sila_directory to authenticated;

comment on table public.doctor_links is
  'Sila — lien professionnel entre deux cabinets Tabibo (demande puis acceptation).';
comment on table public.doctor_referrals is
  'Sila — adressage d''un patient d''un cabinet à un autre. Aucune pièce du dossier n''y transite.';
comment on table public.doctor_notes is
  'Sila — message écrit entre deux confrères reliés, éventuellement rattaché à un adressage.';
