...
export default function History({ state, setState, go, openNewAppt, openAddPatient }) {
  ...
  return (
    <div style={{ padding: isMobile ? 12 : 32, background: BG, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <Hero tint={SEC.histo} icon={ICONS.clock} isMobile={isMobile}
          title="Consultations"
          sub="Le journal de tous vos rendez-vous — date, statut et paiement. Le détail médical d’un patient se trouve dans son dossier."
          chips={[
            { value: filtered.length, label: filtered.length > 1 ? 'consultations' : 'consultation' },
            { value: filtered.filter((c) => c.status === 'Payé').length, label: 'encaissées', color: '#0E7C52' },
            { value: `${filtered.filter((c) => c.status === 'Payé').reduce((s2, c) => s2 + (c.amount || 0), 0).toLocaleString('fr-FR')}`, label: 'MAD', color: SEC.factures.c },
          ]}
          right={<button onClick={exportCSV} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '6px 14px', minHeight: 30, border: `1px solid ${BORDER_STRONG}`, borderRadius: 8, background: '#fff', color: DARK, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
